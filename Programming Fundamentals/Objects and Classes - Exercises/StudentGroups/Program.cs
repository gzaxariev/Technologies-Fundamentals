...
namespace StudentGroups
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine("{0} => {1}", group.Town.Name,
                    string.Join(", ", group.Students.Select(s => s.Email).ToList()));
            }
        }
        ...
        static Student ReadStudent(string input)
        {
            ...
            DateTime registrationDate =
                DateTime.ParseExact(tokens[2].Trim(), "d-MMM-yyyy", CultureInfo.InvariantCulture);
